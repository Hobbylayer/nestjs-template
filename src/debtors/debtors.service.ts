...
export class DebtorsService {
  ...
  async findByCommunity(
    ...
    const totalDocs: number = (
      await this.paymentsRequestModel.aggregate(query).exec()
    ).length;

    // calc total of pages.
    let totalPages: number = Math.trunc(totalDocs / limit);
    if (totalDocs % limit > 0) totalPages++;
    ...
    result = await Promise.all(
      result.map(async (item) => ({
        ...item,
        resident: await this.userModel
          .findOne({ location: item.location._id.toString() })
